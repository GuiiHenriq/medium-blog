...
export default class Posts extends Component {
  ...
  componentWillMount() {
    const POSTS_XML =
      "https://cors-anywhere.herokuapp.com/https://medium.com/feed/@guiihenriq";
    fetch(POSTS_XML, {
      headers: {
  ...
  componentDidUpdate(prevState) {
    if (prevState !== this.state) {
      ...
      for (let i = 0; i < postNodeList.length; i++) {
        ...
        const filterImgPost = postList.elements[10].elements[0].cdata.split(
          '"'
        )[3];
        ...
        for (let y = 0; y < this.state.monthsList.length; y++) {
          if (datePost[2] === this.state.monthsList[y].mesENG) {
            postNodeList[i].childNodes[2].innerHTML =
              datePost[1] +
              " de " +
              this.state.monthsList[y].mesPT +
              " de " +
              datePost[3];
          }
        }
  ...
  convertToJson(apiXML) {
    let array = [];
    let resultJson = convert.xml2js(apiXML, {
      compact: false,
      spaces: 4,
      nativeType: true
    });
    this.setState(
      { dataXml: resultJson.elements[0].elements[0].elements.splice(9) },
      () => {
    ...
    const categoriesJson = this.state.menuCategories.map((post, index) => {
      return (
        <div className="categorie" key={index}>
          <Link to={"/categorias?id=" + post} className="category_post">
            {post}
          </Link>
        </div>
      );
    ...
    const postsJson = this.state.dataXml.map((post, index) => {
      return (
        <div className="post" key={post.elements[2].elements[0].text}>
          ...
          <Link
            to={"/categorias?id=" + post.elements[3].elements[0].cdata}
            className="category_post"
          ></Link>
          <Link
            to={"/post?id=" + post.elements[2].elements[0].text.split("/p/")[1]}
            className="url_post"
          >
            Leia Mais →
          </Link>
        ...
        <section className="header">
          ...
          <div className="search">
            <input
              type="text"
              onChange={() =>
                this.setState({ valueInput: this.inputSearch.current.value })
              }
            ...
            <Link
              ...
              to={"/search?id=" + this.state.valueInput}
            >
              Go!
            </Link>
          </div>
